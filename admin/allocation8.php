    ...
    for ($i = 1; $i <= $totalElectivesAllowed; $i++) {
        ...
        while ($rowStudent = mysqli_fetch_assoc($sqlStudents)) {
            ...
            if (!is_null($electivePref)) {
                ...
                // $seats = $rowSeats['seats_for_MCA'] + $rowSeats['seats_for_MTECH_AI'] + $rowSeats['seats_for_MTECH_CS'] + $rowSeats['seats_for_MTECH_IT'] + $rowSeats['seats_for_IMTECH_3_4'] + $rowSeats['seats_for_IMTECH_5_6'] + $rowSeats['seats_for_IMTECH_7_8'];
                // Fetch available seats for this elective and stream
                $rowSeats = mysqli_fetch_assoc($sqlAvailableSeats);
                $seats = 0;
                ...
                if ($seats > 0) {
                    ...
                    // $sqlAllocateElective = "INSERT INTO courses_allocated (courseName, bucket_number, student_reg_no, stream_name, course_type)
                    ...
                    // Move to the next preference for this student
                    continue;
                } else {
                    // Move to the next preference if seats are not available for the current preference
                    continue;
                }
            } else {
