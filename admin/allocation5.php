...
if (isset($_POST['submit'])) {
    ...
    $sql = mysqli_query($con, "SELECT studentRegno, course_name, optional_core_choice_1, optional_core_choice_2, optional_core_choice_3, optional_core_choice_4, optional_core_choice_5
                                FROM students
                                WHERE stream_id = '$streamId' AND enrolment_status_OptionalCore = 0
                                ORDER BY cgpa DESC");
    if (!$sql) {
    ...
    while ($row = mysqli_fetch_array($sql)) {
        ...
        foreach ($choices as $choice) {
            if ($choice != NULL && $choice != "NULL") {
                ...
                $courseName = mysqli_real_escape_string($con, $row['course_name']);

                $sqlInsert = "INSERT INTO courses_allocated (course_code, student_reg_no, course_name, course_type)
                                VALUES ('$courseCode', '$studentRegNo', '$courseName', 'Optional Core')";
...
if (isset($_POST['submitElective'])) {
    ...
    // Fetching total number of electives for the stream
    ...
    $totalElectives = $rowElectiveCount['electives'];

    // Fetching all students who haven't been allocated electives yet
    $sqlStudents = mysqli_query($con, "SELECT * FROM students WHERE stream_id = '$streamId' AND enrolment_status_elective = 0 ORDER BY cgpa DESC");
    ...
    if (mysqli_num_rows($sqlStudents) == 0) {
    ...
    //     $sqlElectivePrefs = mysqli_query($con, "SELECT * FROM elective_preference WHERE studentRegno = '$studentRegNo' ORDER BY E1, E2, E3, E4, E5, E6, E7, E8, E9, E10, E11, E12, E13, E14, E15, E16, E17, E18, E19");
    ...
    //     while ($rowPref = mysqli_fetch_assoc($sqlElectivePrefs)) {
    //         $prefCourse = $rowPref['course_code'];

    //         // Checking if elective is already allocated to another student
    //         $sqlAllocated = mysqli_query($con, "SELECT * FROM courses_allocated WHERE course_code = '$prefCourse'");
    ...
    //         if (mysqli_num_rows($sqlAllocated) < $totalElectives) {
    //             $courseCode = mysqli_real_escape_string($con, $prefCourse);
    //             $studentRegNo = mysqli_real_escape_string($con, $rowStudent['studentRegno']);
    //             $courseName = mysqli_real_escape_string($con, $rowPref['course_name']);

    //             // Elective not fully allocated, allocate it to current student
    //             $sqlAllocateElective = mysqli_query($con, "INSERT INTO courses_allocated (course_code, student_reg_no, course_name, course_type) 
    //                                                         VALUES ('$courseCode', '$studentRegNo', '$courseName', 'Elective')");
    ...
    //             array_push($selectedElectives, $prefCourse);
    //         }

    //         // Check if required number of electives allocated
    //         if (count($selectedElectives) >= $totalElectives) {
    //             break;
    //         }
    //     }

    //     // Update enrolment status for the student
    ...
    while ($rowStudent = mysqli_fetch_assoc($sqlStudents)) {
        ...
        // Sort the elective preferences based on column order
        asort($electivePrefs);
        ...
        // Loop through each preference of the student
        foreach ($electivePrefs as $prefCourseCode) {
            // Check if the preference is not NULL
            if ($prefCourseCode != NULL && $prefCourseCode != "NULL") {
                // Checking if elective is already allocated to another student
                $sqlAllocated = mysqli_query($con, "SELECT * FROM courses_allocated WHERE course_code = '$prefCourseCode'");
                ...
                if (mysqli_num_rows($sqlAllocated) < $totalElectives) {
                    $courseCode = mysqli_real_escape_string($con, $prefCourseCode);
                    $studentRegNo = mysqli_real_escape_string($con, $rowStudent['studentRegno']);

                    // Get the course name based on the preference order (column name)
                    $prefOrder = array_search($prefCourseCode, array_keys($electivePrefs));
                    $courseName = "E" . $prefOrder;
                    ...
                    // Elective successfully allocated, add to selected electives array
                    array_push($selectedElectives, $prefCourseCode);
                }
    ...
    $sqlDeleteDuplicates = mysqli_query($con, "DELETE FROM courses_allocated WHERE courses_allocated_id NOT IN (SELECT MIN(courses_allocated_id) FROM courses_allocated GROUP BY student_reg_no, course_code)");
