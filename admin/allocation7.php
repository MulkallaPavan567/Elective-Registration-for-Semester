...
if (isset($_POST['submitElective'])) {
    ...
    // Fetch available seats for each course in the respective stream
    $sqlAvailableSeats = mysqli_query($con, "SELECT * FROM course");
    if (!$sqlAvailableSeats) {
        die("Query failed: " . mysqli_error($con));
    }

    // Store available seats in an associative array
    $availableSeats = array();
    while ($rowSeats = mysqli_fetch_assoc($sqlAvailableSeats)) {
        $courseName = $rowSeats['courseName'];
        $availableSeats[$courseName] = $rowSeats['seats_for_MCA'] + $rowSeats['seats_for_MTECH_AI'] + $rowSeats['seats_for_MTECH_CS'] + $rowSeats['seats_for_MTECH_IT'] + $rowSeats['seats_for_IMTECH_3_4'] + $rowSeats['seats_for_IMTECH_5_6'] + $rowSeats['seats_for_IMTECH_7_8'];
    }

    // Fetching all students and their elective preferences
    ...
    while ($rowStudent = mysqli_fetch_assoc($sqlStudents)) {
        ...
        $electivePrefs = array_slice($rowStudent, 3); // Extract elective preferences from the row
        ...
        foreach ($electivePrefs as $prefCourseName) {
            ...
            if ($prefCourseName != NULL && $prefCourseName != "NULL" && count($selectedElectives) < $totalElectivesAllowed) {
                // Check available seats for the course
                if (!isset($availableSeats[$prefCourseName]) || $availableSeats[$prefCourseName] <= 0) {
                    // No available seats for this course, continue to next preference
                    continue;
                }

                // Decrement available seats for the course
                $availableSeats[$prefCourseName]--;
    ...
    // After allocating first electives for all students, update their elective status and reset selected electives array
    $sqlResetElectiveStatus = mysqli_query($con, "UPDATE students SET enrolment_status_elective = 0 WHERE stream_id = '$streamId'");
    if (!$sqlResetElectiveStatus) {
        // Query execution failed, display error
        die("Failed to reset elective status: " . mysqli_error($con));
    }

    $selectedElectives = array(); // Reset selected electives array

    // Loop through each student to allocate second electives
    mysqli_data_seek($sqlStudents, 0); // Reset the result pointer to start from the first row again
    while ($rowStudent = mysqli_fetch_assoc($sqlStudents)) {
        // Your logic to allocate the second elective goes here
        // It should be similar to the first elective allocation logic
        // You can reuse the existing code with necessary adjustments
    }
