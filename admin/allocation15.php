...
if (isset($_POST['submitElective'])) {
    ...
    for ($i = 1; $i <= 19; $i++) {
        ...
        while ($rowStudent = mysqli_fetch_assoc($sqlStudents)) {
            ...
            // Flag to indicate if elective has been allocated for the current student
            $electiveAllocated = false;

            // // Loop through each elective preference for the current student
            // for ($j = $i; $j <= $totalElectivesAllowed; $j++) {
            ...
            if (!is_null($electivePref)) {
                echo "<script>alert('if loop.')</script>";
                ...
                $seats = calculateTotalSeats($rowSeats, $rowStudent['stream_name']);

                // Check if there are available seats
                if ($seats > 0) {
                    // Allocate elective to the student
                    allocateElectiveToStudent($con, $rowStudent, $electivePref, $rowSeats);
                    ...
                    if ($seats > 0 && $seats < 20) {
                        allocateElectiveToStudent($con, $rowStudent, $electivePref, $rowSeats);
                        echo "<script>alert('seats< 0 , Electives allocated1.')</script>";
                        $electiveAllocated = true;
                        break; // Exit the loop once elective is allocated
                    }
                }
        ...
        // If elective was not allocated for this student, move to the next student
...
// Function to calculate total available seats based on the stream
function calculateTotalSeats($rowSeats, $streamName)
{
    $seats = 0;
    switch ($streamName) {
        case 'MCA':
...
// Function to allocate elective to the student
function allocateElectiveToStudent($con, $rowStudent, $electivePref, $rowSeats)
{
    ...
    if (!$sqlAllocateElective) {
        ...
        mysqli_query($con, "UPDATE course SET " . getSeatColumn($streamName) . " = " . getSeatColumn($streamName) . " - 1 WHERE courseName = '$electivePref'");
    }
}

// Function to get seat column based on the stream
function getSeatColumn($streamName)
{
    switch ($streamName) {
        case 'MCA':
            return 'seats_for_MCA';
        case 'MTECH_AI':
            return 'seats_for_MTECH_AI';
        case 'MTECH_CS':
            return 'seats_for_MTECH_CS';
        case 'MTECH_IT':
            return 'seats_for_MTECH_IT';
        case 'IMTECH_3_4':
            return 'seats_for_IMTECH_3_4';
        case 'IMTECH_5_6':
            return 'seats_for_IMTECH_5_6';
        case 'IMTECH_7_8':
            return 'seats_for_IMTECH_7_8';
        default:
            // Handle unexpected stream names
            return '';
    }
